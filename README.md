...
| 성명 | 역할 | 담당업무 |
| 김신형 | 멘토 | 일정 조율, Q&A답변 |
| 임재혁 | 팀장 | Weather_API 기능 구현, Raspberry Pi 코딩 |
| 장민혁 | 팀원 | 차선 탐지 및 객체 탐지 알고리즘 구현, Raspberry Pi 코딩 |
| 조수연 | 팀원 | 차선 탐지 및 객체 탐지 알고리즘 구현, Arduino 코딩 ,프로젝트 시나리오 구현 |
| 진우열 | 멘토 | 차선 탐지 및 객체 탐지 알고리즘 구현, Arduino 코딩 ,프로젝트 시나리오 구현 |
|   | 공통업무 | YOLO 모델 DATASET 제작 및 학습, RC카 제작 |
